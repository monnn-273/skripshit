...
def preprocess_fastapi(image, max_size=512):
    """
    Preprocess an image for EfficientDet model
    Args:
        image: numpy array in RGB format
        max_size: max size for the longer edge
    Returns:
        ori_imgs, framed_imgs, framed_metas
    """
    ori_imgs = [image]
    normalized_imgs = [
        (image / 255 - np.array([0.485, 0.456, 0.406]))
        / np.array([0.229, 0.224, 0.225])
    ]
    imgs_meta = [{"original_size": image.shape[:2]}]

    framed_imgs = []
    framed_metas = []

    for img_id, img in enumerate(normalized_imgs):
        ...
        # Pad to square
        pad_h = max_size - img.shape[0]
        pad_w = max_size - img.shape[1]
        img = np.pad(img, ((0, pad_h), (0, pad_w), (0, 0)), mode="constant")

        framed_imgs.append(img)
        framed_metas.append(
            {
                "original_size": imgs_meta[img_id]["original_size"],
